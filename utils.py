...
def get_user_field(user: str, field: str):
    info_path = os.path.join("Users", user, "info.json")
    try:
...
def generate_cl_prompt(user: str, raw_html: str) -> str:
    ...
    try:
        clres_prompt = os.path.join("Users", user, "prompts", "cover_letter_prompt.txt")
        with open(clres_prompt, "r", encoding="utf-8") as f:
            prompt_template = f.read()
    except FileNotFoundError:
        return "Error: Cover letter prompt template not found."
...
def generate_resume_prompt(user: str, job_title: str, raw_html: str) -> str:
    ...
    resume_prompt = os.path.join("Users", user, "prompts", "resume_prompt.txt")
    with open(resume_prompt, "r", encoding="utf-8") as f:
        prompt_template = f.read()
